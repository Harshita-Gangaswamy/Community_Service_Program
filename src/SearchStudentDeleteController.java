...
public class SearchStudentDeleteController implements Initializable {
	...
	void search(ActionEvent event){
		if(studentID.getText().length() == 7){
			...
			if(info != null){
				DeleteStudentController.setStudentInfo(info);
				try {
				  URL menuBarUrl = getClass().getResource("Menubar.fxml");
				  URL paneUrl = getClass().getResource("deleteStudent.fxml");
